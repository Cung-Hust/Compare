...
class DeviceDataHandler(IHandler):
    ...
    def handler(self, item):
        ...
        try:
            ...
            switcher = {
                ...
                "DeviceEventLog": self.device_event_log,
            }
    ...
    def device_info(self, data):
        ...
        for d in devices:
            ...
            db.Services.DeviceService.UpdateDeviceByCondition(
                db.Table.DeviceTable.c.DeviceAddress == d.get(
                    "Device"), update_data
            )

    def device_event_log(self, data):
        cmd_rsp = {
            "RQI": str(uuid.uuid4())
        }
        cmd_rsp.update(data)
        self.__mqtt.send(
            Const.MQTT_DEVICE_TO_CLOUD_REQUEST_TOPIC, json.dumps(cmd_rsp))
    ...
    def device_report(self, data):
        ...
        cmd_rsp.update(data)
        self.__mqtt.send(
            Const.MQTT_CLOUD_TO_DEVICE_RESPONSE_TOPIC, json.dumps(cmd_rsp))
        # db = Db()
        # gateway_data_update = {
        ...
        #     Const.GATEWAY_ID, gateway_data_update)
        # devices = data.get("Devices")
        # for d in devices:
        #     update_data = {
        #         'KWH': d.get("KWh"),
